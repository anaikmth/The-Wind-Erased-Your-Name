...
func _start_cinematic() -> void:
	...
	t.tween_property(_black_rect, "modulate:a", 0.0, 2.0)
	await t.finished
...
func _play_music() -> void:
	var music_path := "res://assets (photos, musiques...)/Masked Ball (1999 Extended Mix).mp3"
	print("existe : ", ResourceLoader.exists(music_path))
	if not ResourceLoader.exists(music_path):
		print("fichier introuvable, on sort")
		return
	print("chargement...")
	_music = AudioStreamPlayer.new()
	add_child(_music)
	_music.stream = load(music_path)
	print("stream : ", _music.stream)
	_music.volume_db = -80.0
	_music.play()
	print("en lecture : ", _music.playing)
	var tm := create_tween()
	tm.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	tm.tween_property(_music, "volume_db", -10.0, 2.5)


func _stop_music_and_go() -> void:
	...
	fade.tween_property(_black_rect, "modulate:a", 1.0, 1.5)
	await fade.finished

	get_tree().change_scene_to_file("res://scenes/porte_scene.tscn")
